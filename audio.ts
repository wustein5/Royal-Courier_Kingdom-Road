class AudioService {
  ...
  // D Dorian — bright medieval travel feel
  ...
  // E Phrygian — dark ominous boss feel
  ...
  private scheduler() {
    ...
    while (this.nextNoteTime < this.ctx.currentTime + lookAhead) {
      if (this.melodyGain) {
        ...
        this.scheduleNote(freq, dur, this.nextNoteTime, this.melodyGain,
          isBoss ? 'sawtooth' : 'triangle', isBoss ? 0.12 : 0.15);
        // Harmony
        const harmonyRatio = isBoss ? 1.189 : 1.498;
        this.scheduleNote(freq * harmonyRatio, dur, this.nextNoteTime, this.melodyGain,
          isBoss ? 'sawtooth' : 'triangle', isBoss ? 0.05 : 0.07);
  ...
  startMusic(isBoss: boolean = false) {
    ...
    try {
      ...
      // Drone — root note held like a hurdy-gurdy
  ...
  stopMusic() {
    ...
    try {
      if (this.droneGain) {
        const droneOsc = (this.droneGain as any)._droneOsc;
  ...
  playArrow() {
    this.playTone(900, 'sawtooth', 0.04, 0.07);
    this.playTone(500, 'sawtooth', 0.08, 0.05, 0.04);
  }

  playSword() {
    this.playTone(280, 'sawtooth', 0.12, 0.18);
    this.playTone(180, 'square', 0.18, 0.1, 0.08);
    this.playTone(350, 'sine', 0.06, 0.12, 0.02);
  }

  playHit() {
    this.playTone(140, 'square', 0.1, 0.2);
    this.playTone(90, 'sawtooth', 0.14, 0.14, 0.04);
  }

  playCoin() {
    this.playTone(1046, 'sine', 0.08, 0.12);
    this.playTone(1318, 'sine', 0.08, 0.1, 0.08);
    this.playTone(1567, 'sine', 0.1, 0.08, 0.16);
  }

  playUpgrade() {
    [392, 523, 659, 784, 1047].forEach((f, i) => this.playTone(f, 'triangle', 0.18, 0.15, i * 0.1));
  }

  playShopOpen() {
    [523, 659, 784, 659, 784, 1047].forEach((f, i) => this.playTone(f, 'triangle', 0.15, 0.12, i * 0.1));
  }

  playBossWarning() {
    this.playTone(87, 'sawtooth', 0.6, 0.28);
    this.playTone(65, 'sawtooth', 0.6, 0.18, 0.1);
    this.playTone(110, 'square', 0.4, 0.12, 0.3);
  }

  playCarriageBreak() {
    for (let i = 0; i < 8; i++) {
      this.playTone(220 - i * 20, 'sawtooth', 0.15, 0.2, i * 0.06);
    }
    this.playTone(80, 'square', 0.4, 0.3, 0.1);
  }
}
